...
class ArcballControl {
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    const raIntensity = 0.8 * timeScale;
    quat.slerp(this._combinedQuat, this._combinedQuat, combinedQuat, raIntensity);
...
class InfiniteGridMenu {
  ...
  #updateProjectionMatrix(gl) {
    ...
    this.camera.fov =
      this.camera.aspect > 1 ? 2 * Math.atan(height / distance) : 2 * Math.atan(height / this.camera.aspect / distance);
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(canvas, menuItems, handleActiveItem, setIsMoving, sk => sk.run(), scale);
    }
  ...
  return (
    <div className="infinite-menu-shell">
      <canvas id="infinite-grid-menu-canvas" ref={canvasRef} aria-label="Bản đồ lộ trình học tương tác" />
      ...
      {activeItem && (
        <>
          <button
            ...
            aria-label={`Xem ${activeItem.title}`}
          >
